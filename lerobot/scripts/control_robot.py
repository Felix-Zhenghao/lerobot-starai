...
def record(
    ...
    # NOTE: if not teleop (have policy), the aim of warmup_record just does normal steps of inference
    # NOTE: if teleop, the aim of warmup_record is to give time for the follower arm to go to the position of the leader arm
    ...
    go_to_rest_pose(
        robot=robot,
        control_time_s=3,
        control_leader_arm=policy is None,
    )
    recorded_episodes = 0
    while True:
        ...
        if not events["stop_recording"] and (
            ...
            go_to_rest_pose(
                robot=robot,
                control_time_s=3,
                control_leader_arm=True,
            ) # NOTE: this, reset the environment
...
def replay(
    ...
    for idx in range(dataset.num_frames if cut is None else min(cut, dataset.num_frames)):
